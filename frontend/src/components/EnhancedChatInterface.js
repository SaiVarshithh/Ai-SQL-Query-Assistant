...
import {
  ...
  Switch, FormControlLabel, Chip, Fab, Zoom, Slider, FormControl, InputLabel, Input
} from '@mui/material';
...
const CodeCell = ({ code, darkMode }) => {
  ...
  return (
    <Box sx={{ position: 'relative', marginY: 2, maxWidth: '100%', overflowX: 'auto' }}>
          ...
          <TableBody>
            {data.map((row, index) => (
              <TableRow key={index} sx={{ '&:nth-of-type(odd)': { backgroundColor: theme.palette.action.hover } }}>
                {columns.map((column) => (
                  <TableCell key={column} sx={{ whiteSpace: 'nowrap', minWidth: '150px' }}>
...
export default function ProfessionalAIDatabaseAssistant({ connectionInfo, setIsLoggedIn }) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages([
        ...updatedMessages,
        {
          content: 'The input you entered is unable to read, can you check the spelling and retry',
          isUser: false,
        },
      ]);
  ...
  const isButtonEnabled = inputQuery.trim() && selectedDatabase;
